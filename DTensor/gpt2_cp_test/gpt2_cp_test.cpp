...
float get_lr(int step, float max_lr, float min_lr, int warmup_steps, int max_steps) {
    ...
    float coeff = 0.5f * (1.0f + std::cos(static_cast<float>(M_PI) * decay_ratio));
...
public:
    ...
    MLP(int64_t n_embd, int64_t n_layers, DeviceIndex device, uint64_t seed = 1234)
        : ln(n_embd),
...
public:
    ...
    GPT(GPTConfig cfg, DeviceIndex device,
        std::shared_ptr<ProcessGroupNCCL> pg,
        const DeviceMesh& mesh,
        uint64_t seed = 1234)
        : config(cfg),
          wte(cfg.vocab_size, cfg.n_embd, device, seed),
          wpe(cfg.context_length, cfg.n_embd, device, seed),
          ln_f(cfg.n_embd)
    {
        ln_f.to(device);
        ...
        for (int i = 0; i < cfg.n_layers; ++i) {
            ...
            auto m = std::make_shared<MLP>(
                cfg.n_embd, cfg.n_layers, device, seed);
        ...
        if (cfg.weight_tying) {
            ...
            init_linear_gpt2(*lm_head, 0.02f, seed , true);
            lm_head->to(device);
    ...
    void reset_timing() {
        ...
        for (auto& a : attn_blocks) a->reset_t_attn();
    }

    void collect_attn_timing() {
        t_attn = 0.0;
        for (auto& a : attn_blocks) t_attn += a->t_attn;
    }
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // Model
        GPT model(config, device, pg, mesh, /*seed=*/42);
        ...
        const int max_steps    = static_cast<int>(num_params) / global_batch * 5;
            ...
            try {
                ...
                if (step % VAL_FREQ == 0 || step == max_steps - 1) {
                    ...
                    for (int v = 0; v < val_steps; ++v) {
                        ...
                        val_loss_accum += vloss_cpu.data<float>()[0] /
                                          static_cast<float>(val_steps);
                    }
                ...
                if (step > 0 && (step % TOK_GEN_FREQ == 0 || step == max_steps - 1)) {
                    ...
                    Tensor xgen(Shape({{num_return_seq, 8}}),
                                TensorOptions()
                    ...
                    while (xgen.shape().dims[1] < max_length) {
                        ...
                        int64_t padded_len = ((seq_len + world_size - 1) / world_size) * world_size;
                        Tensor xgen_fwd = xgen;
                        if (padded_len != seq_len) {
                            int64_t Bg_pad = xgen.shape().dims[0];
                            Tensor pad = Tensor::zeros(
                                Shape({{Bg_pad, padded_len - seq_len}}),
                                xgen.opts());
                        ...
                        Tensor topk_probs_2d = autograd::reshape(
                            topk_probs, Shape({{Bg, 50}}));
                        ...
                        if (world_size > 1) {
                            ix = ix.contiguous();
                            pg->broadcast(ix.data<int64_t>(), ix.data<int64_t>(),
                                          ix.numel(), Dtype::Int64, 0, true);
                        }

                        Tensor topk_idx_2d =
                            autograd::reshape(topk_idx, Shape({{Bg, 50}}));
                ...
                Tensor loss_accum_gpu = Tensor::zeros(
                    Shape{{1}}, TensorOptions().with_device(device));

                static Tensor grad_scale = Tensor::full(
                    Shape{{1}},
                    TensorOptions().with_device(device),
                ...
                for (int micro = 0; micro < grad_accum_steps; ++micro) {
                    ...
                    //     graph_guard = std::make_unique<autograd::GraphRecordGuard>(true);
                    // }
                ...
                if (model.config.weight_tying && model.lm_head->weight.has_grad()) {
                    Tensor lm_grad_T = model.lm_head->weight.grad_view()
                ...
                float  loss_scalar = loss_cpu.data<float>()[0] /
