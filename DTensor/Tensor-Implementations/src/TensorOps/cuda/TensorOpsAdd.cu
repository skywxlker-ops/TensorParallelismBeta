...
namespace OwnTensor
{
    ...
    void add_kernel_nd_broadcast(const T *a, const T *b, T *output,
                                            ...
                                            size_t ndim, size_t total_elems)
    {
        size_t idx = blockIdx.x * (size_t)blockDim.x + threadIdx.x;
        size_t stride = (size_t)blockDim.x * gridDim.x;
    ...
    void cuda_add_tensor(const Tensor &A, const Tensor &B, Tensor &output, cudaStream_t stream)
    {
        ...
        if (!needs_broadcasting && A.is_contiguous() && B.is_contiguous()) {
            ...
            size_t *d_a_strides, *d_b_strides;
            
            cudaMallocAsync(&d_a_shape, meta.ndim * sizeof(size_t), stream);
            cudaMallocAsync(&d_b_shape, meta.ndim * sizeof(size_t), stream);
            cudaMallocAsync(&d_out_shape, meta.ndim * sizeof(size_t), stream);
            cudaMallocAsync(&d_a_strides, meta.ndim * sizeof(size_t), stream);
            cudaMallocAsync(&d_b_strides, meta.ndim * sizeof(size_t), stream);
            
            cudaMemcpyAsync(d_a_shape, meta.a_shape, meta.ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(d_b_shape, meta.b_shape, meta.ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(d_out_shape, meta.out_shape, meta.ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(d_a_strides, meta.a_strides, meta.ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            cudaMemcpyAsync(d_b_strides, meta.b_strides, meta.ndim * sizeof(size_t), cudaMemcpyHostToDevice, stream);
            
            add_kernel_nd_broadcast<<<grid_size, block_size, 0, stream>>>(
                a_ptr, b_ptr, output_ptr,
                d_a_shape, d_b_shape, d_out_shape,
                d_a_strides, d_b_strides,
                meta.ndim, total_elems
            );
    ...
    void add_inplace_kernel_broadcast(T *lhs, const T *rhs,
                                                 const size_t *lhs_shape, const size_t *rhs_shape, const size_t *out_shape,
