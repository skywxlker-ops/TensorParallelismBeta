...
// ========================================
// TEST SECTION 5: GPU OPERATIONS
// ========================================
void test_gpu_operations() {
    print_test_header("GPU REDUCTION OPERATIONS");
    ...
    try {
        ...
        Tensor result_gpu = reduce_sum(gpu_t, {0}, false);
        Tensor result_cpu = result_gpu.to_cpu();
        auto end = std::chrono::high_resolution_clock::now();
        
        double duration = std::chrono::duration<double, std::milli>(end - start).count();
        
        bool shape_ok = result_cpu.shape().dims[0] == 4;
        log_test("GPU: Sum along axis 0", shape_ok, "", duration);
    } catch (const std::exception& e) {
        log_test("GPU: Sum reduction", false, e.what());
    }
    
    // Test 2: GPU Mean (Bfloat16)
    try {
        ...
        Tensor gpu_t = cpu_t.to_cuda(0);
        Tensor result_gpu = reduce_mean(gpu_t, {0}, false);
        Tensor result_cpu = result_gpu.to_cpu();
        
        log_test("GPU: Mean (Bfloat16)", result_cpu.shape().dims[0] == 3);
    } catch (const std::exception& e) {
        log_test("GPU: Mean (Bfloat16)", false, e.what());
    }
    ...
    try {
        ...
        Tensor gpu_t = cpu_t.to_cuda(0);
        Tensor result_gpu = reduce_argmax(gpu_t, {1}, false);
        Tensor result_cpu = result_gpu.to_cpu();
        
        int64_t* res = result_cpu.data<int64_t>();
        bool passed = (res[0] == 2) && (res[1] == 0);
        log_test("GPU: ArgMax", passed);
    } catch (const std::exception& e) {
        log_test("GPU: ArgMax", false, e.what());
    }
