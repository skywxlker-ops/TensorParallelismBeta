...
/**
 * 10M Parameter MLP Block for GPU (Without Checkpointing)
 * -------------------------------------------------------
 * Hidden Size (H) = 1024
...
void run_gpu_test_no_cp(DeviceIndex device) {
    if (!device.is_cuda()) {
    ...
    tracker.init("memory_test_gpu_10m_no_cp.csv");
    ...
    variable_list out = model.forward({x});
...
int main() {
    try {
        ...
        if (OwnTensor::device::cuda_available()) {
            run_gpu_test_no_cp(DeviceIndex(Device::CUDA, 0));
        } else {
