   ...
   int batchCount)
{
   ...
   size_t smem_size = 3 * (BM * BK + BK * BN) * sizeof(__half);
  
   static bool init = false;
   if (!init) {
       cudaFuncSetAttribute(hgemm_async_strided_batched_kernel,
                          cudaFuncAttributeMaxDynamicSharedMemorySize,
                          (int)smem_size);
       init = true;
   }
  
   hgemm_async_strided_batched_kernel<<<grid, block, smem_size, stream>>>(
       M, N, K, alpha,
       A, lda, strideA,
       B, ldb, strideB,
       beta, C, ldc, strideC
