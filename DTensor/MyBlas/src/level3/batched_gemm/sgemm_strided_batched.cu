...
// Strided Batched GEMM using TF32 Tensor Cores
// Grid-level batching: blockIdx.z represents batch index
#define BM 128
#define BN 128
#define BK 16
...
__global__ void wmma_tf32_gemm_strided_batched_kernel(
    ...
    // WMMA accumulators (4x4 = 16 fragments per warp)
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            int row = by * BM + warp_y * 64 + i * 16;
            int col = bx * BN + warp_x * 64 + j * 16;
    ...
    int batchCount)
{
    ...
    // Grid dimensions: (N blocks, M blocks, batch count)
    dim3 grid((N + BN - 1) / BN, (M + BM - 1) / BM, batchCount);
    dim3 block(THREADS_PER_BLOCK);
    
    size_t smem_size = (3*BM*BK + 3*BK*BN) * sizeof(float);
    
    // Set shared memory attribute (only once)
    static bool init = false;
    if (!init) {
        cudaFuncSetAttribute(wmma_tf32_gemm_strided_batched_kernel, 
                           cudaFuncAttributeMaxDynamicSharedMemorySize, 
                           (int)smem_size);
        init = true;
    }
    
    wmma_tf32_gemm_strided_batched_kernel<<<grid, block, smem_size, stream>>>(
        A, lda, strideA,
        B, ldb, strideB,
        C, ldc, strideC,
        M, N, K, alpha, beta
