...
public:
          ...
          lm_head(cfg.n_embd, cfg.vocab_size, false)
    {
        ...
        // Ensure lm_head is correctly wrapped as a replicated DTensor for clipping
        register_parameter(&lm_head.weight);
        if(lm_head.bias.is_valid()) register_parameter(&lm_head.bias);
    }
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // Print parameter count (global, not local)
        std::vector<DTensor*> params = model.parameters();
        int64_t num_params = 0;
        for(auto& p : params) {
            int64_t n = p->mutable_tensor().numel();
            // If layout is sharded, multiply by world_size to get global count
            if (p->get_layout().is_sharded()) {
                n *= world_size;
            }
            num_params += n;
        }
        const int max_steps = num_params * 5 / global_batch;
        // const int max_steps = 10; // Capped for profiling
        ...
        DataLoaderLite train_loader(B, T, 0, 1, "train", data_root, pg->get_rank() == 0, 100000000, rank);
        DataLoaderLite val_loader(B, T, 0, 1, "val", data_root, pg->get_rank() == 0, 100000000, rank);
        ...
        for (int step = 0; step < max_steps; ++step) {
            char step_tag[32];
            sprintf(step_tag, "step_%d", step);
            // nvtxRangePushA(step_tag);
            timer_step.start_timer();
            
            // Validation every 100 steps
            if (step % 100 == 0 || step == max_steps - 1) {
                // nvtxRangePushA("validation");
                val_loader.reset();
                ...
                for (int val_step = 0; val_step < val_loss_steps; ++val_step) {
                    ...
                    // nvtxRangePushA("val_loss_to_cpu");
            ...
            for (int micro_step = 0; micro_step < grad_accum_steps; ++micro_step) {
                // nvtxRangePushA("micro_step");
                
                timer_data.start_timer();
                // nvtxRangePushA("data_loading");
                ...
                // nvtxRangePop(); // micro_step
            }
            
            // nvtxRangePushA("loss_sync_to_cpu");
            ...
            // Prevents numerical drift (e.g., from atomicAdd in Embedding) from compounding over time
            for (auto* p : params) {
                if (p->get_layout().is_replicated() && p->mutable_tensor().has_grad()) {
                    ...
                    if (grad.is_valid() && grad.numel() > 0) {
                        pg->all_reduce_async(grad.data<float>(), grad.data<float>(), grad.numel(), OwnTensor::Dtype::Float32, sum, false)->wait();
