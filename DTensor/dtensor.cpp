...
void worker(int rank, int world_size, const ncclUniqueId &id) {
    ...
    int chunk_size = dt.sliceSize() / world_size;
    std::vector<float*> recv_chunks(world_size);
    for (int i = 0; i < world_size; ++i) cudaMalloc(&recv_chunks[i], chunk_size * sizeof(float));

    for (int i = 0; i < world_size; ++i)
        pg.reduce_scatter(recv_chunks[i], dt.deviceSlice(i), chunk_size, ncclFloat32)->wait();

    for (int i = 0; i < world_size; ++i)
        cudaMemcpy(dt.deviceSlice(i), recv_chunks[i], chunk_size * sizeof(float), cudaMemcpyDeviceToDevice);
    ...
    std::vector<float*> gathered_chunks(world_size);
    for (int i = 0; i < world_size; ++i) {
        cudaMalloc(&gathered_chunks[i], chunk_size * world_size * sizeof(float));
        pg.all_gather(gathered_chunks[i], dt.deviceSlice(i), chunk_size, ncclFloat32)->wait();
        cudaMemcpy(dt.deviceSlice(i), gathered_chunks[i], chunk_size * world_size * sizeof(float), cudaMemcpyDeviceToDevice);
        cudaFree(gathered_chunks[i]);
    }
    ...
    for (auto ptr : recv_chunks) cudaFree(ptr);
}

// ---------------- Main ----------------
#ifndef BINDINGS_MODE
int main() {
    int world_size = 2;
    ncclUniqueId id;
    ncclGetUniqueId(&id);
