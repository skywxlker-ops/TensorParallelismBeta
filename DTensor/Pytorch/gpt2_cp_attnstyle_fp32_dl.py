...
# ══════════════════════════════════════════════════════════════════════════════
# Data Loader — NATIVE torch.utils.data.DataLoader
# ══════════════════════════════════════════════════════════════════════════════
# This replaces the hand-rolled DataLoaderLite with PyTorch's native
# `torch.utils.data.Dataset` + `torch.utils.data.DataLoader`. The batches it
# produces are BYTE-IDENTICAL (same order, same contents) to the old
# DataLoaderLite, so this stays a valid parity reference against the C++ build:
#
#   DataLoaderLite batch n used a flat window tokens[n*B*T : n*B*T + B*T + 1],
#   reshaped to x=buf[:-1].view(B,T), y=buf[1:].view(B,T). Row r of that batch
#   is therefore the contiguous block at offset (n*B + r)*T with a +1-shifted
#   target. We expose exactly that block as one Dataset item k -> offset k*T,
#   then let DataLoader(batch_size=B, shuffle=False, drop_last=True) regroup B
#   consecutive blocks per batch. Identity floor(floor((N-1)/T)/B) ==
#   floor((N-1)/(B*T)) guarantees the same number of full batches per shard,
#   and shuffle=False keeps the order; drop_last=True reproduces the old
#   "abandon the partial tail window, advance to next shard" behaviour.
...
class TokenBlockDataset(Dataset):
    """One shard's token stream exposed as non-overlapping length-T blocks.

    Item k -> (x, y) where
        x = tokens[k*T     : k*T + T]
        y = tokens[k*T + 1 : k*T + T + 1]
    i.e. next-token-prediction pairs. Mirrors DataLoaderLite's contiguous
    packing one row at a time.
    """
    def __init__(self, tokens: torch.Tensor, T: int):
        self.tokens   = tokens
        self.T        = T
        # Number of length-T blocks that also have a valid +1-shifted target.
        self.n_blocks = (len(tokens) - 1) // T
    ...
    def __getitem__(self, k):
        i = k * self.T
        x = self.tokens[i:     i + self.T]
        y = self.tokens[i + 1: i + 1 + self.T]
        return x, y


class NativeShardLoader:
    """Drop-in replacement for DataLoaderLite backed by torch DataLoader.

    Keeps the exact same external contract — next_batch() returns CPU tensors
    of shape [B, T], reset() rewinds to shard 0 — so the training loop is
    unchanged. Internally it drives one native DataLoader per shard and cycles
    shards forever, never letting a batch straddle a shard boundary (matching
    DataLoaderLite, which reset position to 0 on each new shard).
    """
    def __init__(self, B, T, split, num_workers=0, pin_memory=False):
        self.B = B
        self.T = T
    ...
    def _new_loader(self):
        tokens = load_tokens(self.shards[self.current_shard])
        dataset = TokenBlockDataset(tokens, self.T)
        self.loader = DataLoader(
            dataset,
            batch_size=self.B,
            shuffle=False,      # preserve token-stream order (parity)
            drop_last=True,     # drop the partial tail window per shard
    ...
    def next_batch(self):
        try:
            ...
            # Current shard exhausted -> advance and wrap, then start fresh.
...
train_loader = NativeShardLoader(B, T, "train")
val_loader   = NativeShardLoader(B, T, "val")
