...
public:
    ...
    Tensor forward(const Tensor& idx) override {
        ...
        // Output projection: lm_head->forward handles both weight-tied and independent cases
        // When weight_tying: lm_head->weight is a transposed view of wte.weight (no extra autograd node)
        // When not weight_tying: lm_head has its own independent weight
...
int main() {
    try {
        ...
        const int GLOBAL_BATCH = 131072;  // Global batch size
        const int GRAD_ACCUM_STEPS = GLOBAL_BATCH / (B * T);
        
        const float MAX_LR = 6e-4f;  
        const float MIN_LR = MAX_LR * 0.1f;
        const int WARMUP_STEPS = 1034;
        const int MAX_STEPS = 20690;

        const int VAL_FREQ = 1000;
        const int TOK_GEN_FREQ = 2000;
        const int CKPT_FREQ = 2000;
        ...
        ckpt_manager.set_save_intervals(CKPT_FREQ); // Save every 100 steps
        ...
        // Create CSV log file
        std::ofstream log_file("../Training_Logs/master1_throughput_10Kstep_2epoch_prompt.csv");
        ...
        for (int step = start_step; step < MAX_STEPS; ++step) {
            ...
            if(step % TOK_GEN_FREQ == 0 || step == MAX_STEPS - 1) { // More frequent for testing
                ...
                Tensor xgen = Tensor(Shape({{num_return_sequence, 2}}), 
                                           TensorOptions().with_dtype(Dtype::Int64).with_device(device));
                std::vector<int64_t> xgen_tokens = {18165, 318,18165, 318,18165, 318,18165, 318};
                xgen.set_data(xgen_tokens);
                
                uint64_t gen_seed = 42 + rank;  // Advancing seed for diverse sampling
                float temperature = 0.8f;        // Temperature for softmax scaling
                while (xgen.shape().dims[1] < max_length) {
                    ...
                    // to control sharpness of the distribution
                    ...
                    // Use advancing seed so each generation step samples differently
