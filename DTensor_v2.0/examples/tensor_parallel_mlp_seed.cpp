...
int main(int argc, char** argv) {
    ...
    // Pre-compute fixed data for all tensors (SAME as mlp_seed.cpp)
    ...
    std::vector<float> b1_data(B * T * (F/2), 0.0f);   // Zero biases (sharded size for TP)
    std::vector<float> b2_data(B * T * C, 0.0f);
    std::vector<float> b3_data(B * T * (F/2), 0.0f);
    ...
    if (rank == 0) W1.setData(w1_data);  // Set fixed data BEFORE sharding
    ...
    B1.setData(b1_data);  // Set fixed data (zeros) - already correct size per GPU

    // Tensors are already initialized with random data in constructor
    
    // Enable gradient tracking on weight tensors for backward pass
    W1_Shard.mutable_tensor().set_requires_grad(true);
    ...
    H.linear_w_autograd(X, W1_Shard, B1);
    ...
    B2.setData(b2_data);  // Set fixed data (zeros)
    
    // Tensors are already initialized with random data in constructor
    
    // Enable gradient tracking on weight tensors for backward pass
    W2_Shard.mutable_tensor().set_requires_grad(true);
    ...
    // Use autograd-enabled linear for gradient tracking
    Y1.linear_w_autograd(H, W2_Shard, B2);
    ...
    Layout w3_layout(device_mesh, {B, C, F}, 2);
    DTensor W3(device_mesh, pg, w3_layout);
    if (rank == 0) W3.setData(w3_data);  // Set fixed data BEFORE sharding
    W3.replicate(0);
    ...
    Layout h2_layout(device_mesh, {B, T, (F/2)});
    DTensor H2(device_mesh, pg, h2_layout);
    DTensor B3(device_mesh, pg, h2_layout);
    B3.setData(b3_data);  // Set fixed data (zeros)
    
    W3_Shard.mutable_tensor().set_requires_grad(true);
    ...
    H2.linear_w_autograd(Y1, W3_Shard, B3);
    ...
    Layout w4_layout(device_mesh, {B, F, C}, 1);
    DTensor W4(device_mesh, pg, w4_layout);
    if (rank == 0) W4.setData(w4_data);  // Set fixed data BEFORE sharding
    W4.replicate(0);
    ...
    B4.setData(b4_data);  // Set fixed data (zeros)
    
    W4_Shard.mutable_tensor().set_requires_grad(true);
    ...
    // Y2 = H2 @ W4_Shard + B4
    Y2.linear_w_autograd(H2, W4_Shard, B4);
    
    if (rank == 0) std::cout << "\n Y2 before sync (partial sum) \n";
    Y2.display();
    ...
    if (rank == 0) {
        ...
        std::cout << "\n--- Block 1 Gradients ---\n";
        if (W1_Shard.local_tensor().owns_grad()) {
            std::cout << "\nW1_Shard gradient:\n";
            W1_Shard.local_tensor().grad_view().display();
        } else {
            std::cout << "W1_Shard has no gradient (not a leaf or requires_grad=false)\n";
        }
        
        if (W2_Shard.local_tensor().owns_grad()) {
            std::cout << "\nW2_Shard gradient:\n";
            W2_Shard.local_tensor().grad_view().display();
        } else {
            std::cout << "W2_Shard has no gradient (not a leaf or requires_grad=false)\n";
        }
        
        std::cout << "\n--- Block 2 Gradients ---\n";
        if (W3_Shard.local_tensor().owns_grad()) {
            std::cout << "\nW3_Shard gradient:\n";
            W3_Shard.local_tensor().grad_view().display();
        } else {
            std::cout << "W3_Shard has no gradient (not a leaf or requires_grad=false)\n";
        }
        
        if (W4_Shard.local_tensor().owns_grad()) {
            std::cout << "\nW4_Shard gradient:\n";
            W4_Shard.local_tensor().grad_view().display();
        } else {
            std::cout << "W4_Shard has no gradient (not a leaf or requires_grad=false)\n";
        }
    }
