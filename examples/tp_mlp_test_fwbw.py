...
def main():
 ...
 # --- All-reduce partials on device ---
    ...
    # Seed gradient from loss = sum(Z) -> dL/dZ = 1
    dZ = np.ones_like(Z, dtype=np.float32)
    print_debug(rank, "dZ ", dZ)
    
    # ---- gradients w.r.t. second linear (row-parallel) ----
    # Z = H_i @ W2_i  (partial) then all-reduced to full Z
    # For each rank, we have full 'dZ' (because forward all-reduced Z)
    # Local gradient for W2 shard:
    #   dW2_i = H_i^T @ dZ
    dW2_i = H_i.T @ dZ       # shape: (F_per, H)
    print_debug(rank, "dW2_i", dW2_i)
    
    # gradient for bias b2 (replicated)
    db2_local = np.sum(dZ, axis=0)   # shape (H,)
    print_debug(rank, "db2_local", db2_local)
    
    # gradient w.r.t. H_i (local)
    #   dH_i = dZ @ W2_i.T
    dH_i = dZ @ W2_i.T       # shape: (B, F_per)
    print_debug(rank, "dH_i", dH_i)
    
    # ---- GELU backward (shard-wise) ----
    # Y_i was the pre-activation: Y_i = X @ W1_i
    # H_i = gelu(Y_i)
    dY_i = dH_i * gelu_prime(Y_i)   # elementwise
    print_debug(rank, "dY_i (post-gelu-prime)", dY_i)
    
    # bias gradient for b1 shard
    db1_i = np.sum(dY_i, axis=0)    # shape: (F_per,)
    print_debug(rank, "db1_i", db1_i)
    
    # ---- gradients w.r.t. first linear (column-parallel) ----
    # dW1_i = X^T @ dY_i   (local shard gradient)
    dW1_i = X.T @ dY_i               # shape: (H, F_per)
    print_debug(rank, "dW1_i", dW1_i)
    
    # dX_i (local contribution to replicated X)
    # dX_i = dY_i @ W1_i.T
    dX_i = dY_i @ W1_i.T             # shape: (B, H)
    print_debug(rank, "dX_i (local)", dX_i)
    
    # ---- All-reduce dX_i across tensor-parallel ranks to get final dX ----
    # Create DTensor from dX_i flatten and all-reduce in-place (sum)
