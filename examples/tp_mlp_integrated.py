...
def _ptr(t):
    return t.device_ptr() if hasattr(t, "device_ptr") else t.deviceData()
...
def main():
    ...
    try:
        ...
        pg.gemm_strided_batched_f32(
            _ptr(dZ_d), _ptr(W2_T_d), _ptr(dH_d),
            T, F_per, H,
            T*H, 0, T*F_per, B
        )
        ...
        pg.gemm_strided_batched_f32(
            _ptr(dY_d), _ptr(W1_T_d), _ptr(dX_d),
            T, H, F_per,
            T*F_per, 0, T*H, B
        )
        ...
        pg.gemm_f32_rowmajor(_ptr(H1_T_d), _ptr(dZ_flat_d), _ptr(dW2_d), F_per, H, BT,False,False)                                                   
        ...
        pg.gemm_f32_rowmajor(_ptr(X_T_d), _ptr(dY_flat_d), _ptr(dW1_d), H, F_per, BT,False,False)
