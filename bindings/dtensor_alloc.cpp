#include "dtensor_alloc.hpp"
#include "cachingAllocator.hpp"   // use caching allocator
#include <cstring>
#include <stdexcept>
#include <iostream>
...
// Row-major GEMM using cuBLAS (column-major under the hood).
                                     ...
                                     int m, int n, int k, bool transA, bool transB)
{
  ...
  // In row-major view, leading dims:
  ...
  // Swap operands for cuBLAS col-major math:
...
DTensor::DTensor(int world_size, int slice_size, int rank)
  ...
  // Device allocation via caching allocator
  {
...
void DTensor::copyDeviceToHost() {
  CUDA_CHECK(cudaMemcpy(h_data_.data(), d_data_, bytes_, cudaMemcpyDeviceToHost));
}
void DTensor::copyFromHost(const float* src) {
  CUDA_CHECK(cudaMemcpy(d_data_, src, bytes_, cudaMemcpyHostToDevice));
}

// -------- Allocator stats helpers (global) --------
std::string allocator_stats() {
  return CachingAllocator::instance().stats_string();
}
void allocator_print_stats() {
  CachingAllocator::instance().print_stats();
}

// -------- dt::-qualified aliases (for bindings that expect dt::...) --------
namespace dt {
  std::string allocator_stats() {
    return CachingAllocator::instance().stats_string();
  }
